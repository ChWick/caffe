...
class WarpCTCLossLayerTest : public MultiDeviceTest<TypeParam> {
  typedef typename TypeParam::Dtype Dtype;

 protected:
  WarpCTCLossLayerTest()
      : T_(4),
        N_(2),
        num_labels_(3),
  ...
  void InitConstantCorrect() {
    ...
    data[blob_bottom_data_->offset(0, 0, 1)] = one;
    data[blob_bottom_data_->offset(1, 0, 2)] = one;
    data[blob_bottom_data_->offset(2, 0, 0)] = one;
    data[blob_bottom_data_->offset(3, 0, 0)] = one;

    data[blob_bottom_data_->offset(0, 1, 1)] = one;
    data[blob_bottom_data_->offset(1, 1, 0)] = one;
    data[blob_bottom_data_->offset(2, 1, 2)] = one;
    data[blob_bottom_data_->offset(3, 1, 0)] = one;
  ...
  void InitConstantEqual(int sequence_length, int T = 5, int C = 10) {
    ...
    for (int t = 0; t < sequence_length; ++t) {
      label[blob_bottom_label_->offset(t, 0)] = label_to_set + 1;
  ...
  void InitFullRandom(int T = 41, int N = 29, int C = 37, int L = 20, int blank_label = -1) {
    ...
    if (blank_label == -1) {
      ...
      lufp.set_min(blank_label + 1.0001);
      lufp.set_max(num_labels_ - 0.0001);
    }
  ...
  void InitRandom(int T = 41, int N = 29, int C = 37) {
    // This will create random data and random labels for all blobs.
  ...
  void TestForward() {
    ...
    WarpCTCLossLayer<Dtype> layer_1(layer_param);
  ...
  void TestForwardEqual() {
    ...
    InitConstantEqual(sequence_length, 8, 3);
    //InitRandom(8, 1, 3);
    LayerParameter layer_param;
    layer_param.mutable_ctc_loss_param()->set_blank_index(0);
  ...
  void TestGradient() {
    ...
    const Dtype gradient_log_prob_0[5 * 6] =
      {0.0260553, -0.366234, 0.221185, 0.0917319, 0.0129757, 0.0142857,
       0.010436, 0.111121, -0.411608, 0.278779, 0.0055756, 0.00569609,
       0.0037688, 0.0357786, 0.633813, -0.678582, 0.00249248, 0.00272882,
       0.00331533, 0.0663296, -0.356151, 0.280111, 0.00283995, 0.0035545,
       0.00623107, -0.541765, 0.396634, 0.123377, 0.00648837, 0.00903441};

    const Dtype targets_1[4] = {0, 1, 1, 0};
    const Dtype loss_log_prob_1 = -5.42262;

    const Dtype input_prob_matrix_1[5 * 6] =
      {0.161508, 0.30176, 0.28562, 0.0831517, 0.0862751, 0.0816851,
       0.19549, 0.24082, 0.397533, 0.0557226, 0.0546814, 0.0557528,
       0.202456, 0.230246, 0.450868, 0.0389607, 0.038309, 0.0391602,
       0.190345, 0.280884, 0.429522, 0.0326593, 0.0339046, 0.0326856,
       0.154046, 0.423286, 0.315517, 0.0338439, 0.0393744, 0.0339315};
    ...
    const Dtype gradient_log_prob_1[5 * 6] =
      {0.161508, -0.69824, 0.28562, 0.0831517, 0.0862751, 0.0816851,
       0.19549, 0.24082, -0.602467, 0.0557226, 0.0546814, 0.0557528,
       -0.797544, 0.230246, 0.450868, 0.0389607, 0.038309, 0.0391602,
       0.190345, 0.280884, -0.570478, 0.0326593, 0.0339046, 0.0326856,
       0.154046, -0.576714, 0.315517, 0.0338439, 0.0393744, 0.0339315};
    ...
    LayerParameter layer_param;
    WarpCTCLossLayer<Dtype> layer(layer_param);
  ...
  void TestTiming(int T, int N, int L, int C) {
    InitFullRandom(T, N, C, L);
    LayerParameter layer_param;
  ...
  void CompareCTCImplementations() {
    const int blank_index = 0;
    InitFullRandom(4, 1, 3, 2, blank_index);

    Blob<Dtype> origin;
    origin.CopyFrom(*blob_bottom_data_, false, true);
    origin.CopyFrom(*blob_bottom_data_, true, true);

    vector<bool> prob_down(0, false);
    prob_down[0] = true;

    LayerParameter layer_param;
    layer_param.mutable_ctc_loss_param()->set_blank_index(blank_index);

    WarpCTCLossLayer<Dtype> layer_1(layer_param);
    layer_1.SetUp(this->blob_bottom_vec_, this->blob_top_vec_);
    for (int i = 0; i < 10; ++i) {
      blob_top_loss_->mutable_cpu_data()[0] = 0;
      caffe_set<Dtype>(this->blob_bottom_data_->count(), 0.0, this->blob_bottom_data_->mutable_cpu_diff());
      layer_1.Forward(this->blob_bottom_vec_, this->blob_top_vec_);
      layer_1.Backward(this->blob_top_vec_, prob_down, this->blob_bottom_vec_);
      this->blob_bottom_data_->Update();
    }

    // copy data and diff
    Blob<Dtype> backup;
    backup.CopyFrom(*blob_bottom_data_, false, true);
    backup.CopyFrom(*blob_bottom_data_, true, true);

    blob_bottom_data_->CopyFrom(origin, false, true);
    blob_bottom_data_->CopyFrom(origin, true, true);

    const Dtype loss_1 = blob_top_loss_->cpu_data()[0];

    CTCLossLayer<Dtype> layer_2(layer_param);
    layer_2.SetUp(this->blob_bottom_vec_, this->blob_top_vec_);
    for (int i = 0; i < 10; ++i) {
      blob_top_loss_->mutable_cpu_data()[0] = 0;
      caffe_set<Dtype>(this->blob_bottom_data_->count(), 0.0, this->blob_bottom_data_->mutable_cpu_diff());
      layer_2.Forward(this->blob_bottom_vec_, this->blob_top_vec_);
      layer_2.Backward(this->blob_top_vec_, prob_down, this->blob_bottom_vec_);
      this->blob_bottom_data_->Update();
    }

    const Dtype loss_2 = blob_top_loss_->cpu_data()[0];

    EXPECT_LE(std::abs(loss_1 - loss_2), 0.000001);
    const Dtype *data_1 = backup.cpu_data();
    const Dtype *data_2 = blob_bottom_data_->cpu_data();
    const Dtype *diff_1 = backup.cpu_diff();
    const Dtype *diff_2 = blob_bottom_data_->cpu_diff();
    for (int i = 0; i < backup.count(); ++i) {
      EXPECT_LE(std::abs(data_1[i] - data_2[i]), 0.000001);
      EXPECT_LE(std::abs(diff_1[i] - diff_2[i]), 0.000001);
    }
  }
...
TYPED_TEST_CASE(WarpCTCLossLayerTest, TestDtypesAndDevices);

TYPED_TEST(WarpCTCLossLayerTest, TestForward) {
  this->TestForward();
}

TYPED_TEST(WarpCTCLossLayerTest, TestForwardWrong) {
  this->TestForwardWrong();
}

TYPED_TEST(WarpCTCLossLayerTest, TestForwardEqual) {
  this->TestForwardEqual();
}

TYPED_TEST(WarpCTCLossLayerTest, TestGradient) {
  this->TestGradient();
}

/*TYPED_TEST(WarpCTCLossLayerTest, TestTiming_150_1_40_28) {
  this->TestTiming(150, 1, 40, 28);
}

TYPED_TEST(WarpCTCLossLayerTest, TestTiming_150_16_40_28) {
  this->TestTiming(150, 16, 40, 28);
}

TYPED_TEST(WarpCTCLossLayerTest, TestTiming_150_32_40_28) {
  this->TestTiming(150, 32, 40, 28);
}

TYPED_TEST(WarpCTCLossLayerTest, TestTiming_150_64_40_28) {
  this->TestTiming(150, 64, 40, 28);
}

TYPED_TEST(WarpCTCLossLayerTest, TestTiming_150_128_40_28) {
  this->TestTiming(150, 128, 40, 28);
}

TYPED_TEST(WarpCTCLossLayerTest, TestTiming_150_1_20_5000) {
  this->TestTiming(150, 1, 20, 5000);
}

TYPED_TEST(WarpCTCLossLayerTest, TestTiming_150_16_20_5000) {
  this->TestTiming(150, 16, 20, 5000);
}

TYPED_TEST(WarpCTCLossLayerTest, TestTiming_150_32_20_5000) {
  this->TestTiming(150, 32, 20, 5000);
}

TYPED_TEST(WarpCTCLossLayerTest, TestTiming_150_64_20_5000) {
  this->TestTiming(150, 64, 20, 5000);
}

TYPED_TEST(WarpCTCLossLayerTest, TestTiming_150_128_20_5000) {
  this->TestTiming(150, 128, 20, 5000);
}

TYPED_TEST(WarpCTCLossLayerTest, TestTiming_1000_64_200_41) {
  this->TestTiming(1000, 64, 200, 41);
}*/

TYPED_TEST(WarpCTCLossLayerTest, CompareCTCImplementations) {
  this->CompareCTCImplementations();
}
